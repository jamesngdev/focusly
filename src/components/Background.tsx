...
const Background = () => {
    ...
    useEffect(() => {
        const fetchBackground = async () => {
            ...
            const response = await fetch(URL).then(res => res.json()).catch(err => console.error(err));

            const urls = response?.results?.map((result: any) => result?.urls?.full).filter(Boolean);
            setBackgrounds(urls);
