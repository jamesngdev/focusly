...
const Quizlet = () => {
  const [quizletSetId] = useLocalStorage(
  ...
  return (
    <div className="w-[400px] p-4 rounded-lg bottom-4 left-[50%] translate-x-[-50%] translate-y-[-50%] fixed text-white text-center">
      {activeCard && (
        <>
          <div className="flex justify-center items-end gap-2">
            <h3
              className="text-3xl font-bold w-full cursor-pointer"
              onClick={() => pronunciation(activeCard)}
            >
              {activeCard?.front}
            </h3>
          </div>
